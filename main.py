...
def fetch_team_ics(team_id: str, team_name: str):
    ...
    ics_content = [
        ...
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        f"X-WR-CALNAME:{team_name}赛程日历",
        f"X-WR-CALDESC:{team_name}最新赛程及比分同步",
    ...
    for match in matches:
        try:
            ...
            date_str = date_node.text.strip() # 格式 "02-16 04:00"
...
@app.get("/")
def home():
    return {"status": "success", "message": "Football Calendar API is running."}
...
def get_calendar(team_id: str, team_name: str = "球队"):
    ...
    # URL 编码文件名，防止 Safari 等浏览器解析异常
    safe_filename = quote(f"{team_name}.ics")
    
    # 在 main.py 的 @app.get("/api/calendar") 中
    return Response(
        content=ics_data, 
        media_type="text/calendar; charset=utf-8",
        headers={
            # 必须是 inline，不能是 attachment！
            # inline 告诉浏览器：直接处理这个日历，不要下载它
